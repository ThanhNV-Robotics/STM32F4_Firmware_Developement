...
//		uint16_t RegisterAddress; // Register of the address want to read/write
		...
		float IntergraError;
		float AccZ;
		float AccRef = -9.7; // Reference accleraion, initial value is -9.8
		float PreAccRef;
		float PreError; //
...
//float LinearGeneration (int Time, float slope, int InitialValue, int Min, int Max)
//	// _epsilon: 
//	// time in milisecond
//{
//	float ReturnValue;
...
bool PullAndDrop ()
{
	...
	else
	{
		...
		if (StartPulling && !StartDropping) // Pulling Task
		{
			...
			else // Start accelerated pulling
			{
					...
					//AccRef = LinearGeneration(RunningTime,-2,PreAccRef,-9.6-PullingAccel, PreAccRef); // ramping the reference signal
					if (IsOpenLoopControl) // Use open-loop control
					{
						SpeedCmd = LinearSpeedGeneration(RunningTime,-PullingEpsilonAcc,0,-PullingMaxSpeed,0);// Feedforward term
					}
	...
	//				PIDSpeedCmd = PIDCalculate(AccRef,AccZ,-850,850, true); // PID term
				...
				{				
					...
					//AccRef =  = -9.8+PullingDecel;
					AccRef = LinearSpeedGeneration(RunningTime,12,PreAccRef,PreAccRef, -9.8+PullingDecel);
